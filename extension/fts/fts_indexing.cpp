...
static string GetFTSSchemaName(const QualifiedName &qname) {
	return StringUtil::Format("fts_%s_%s", qname.schema.GetIdentifierName(), qname.name.GetIdentifierName());
}

static string GetFTSSchema(const QualifiedName &qname) {
	auto result = IsInvalidCatalog(qname.catalog) ? string("") : qname.catalog.GetIdentifierName() + ".";
...
static string TokenizeMacroScript(const string &ignore, bool strip_accents, bool lower) {
...
string FTSIndexing::DropFTSIndexQuery(ClientContext &context, const FunctionParameters &parameters) {
	...
	if (!Catalog::GetSchema(context, qname.catalog, Identifier(GetFTSSchemaName(qname)),
...
string FTSIndexing::CreateFTSIndexQuery(ClientContext &context, const FunctionParameters &parameters) {
	...
	auto get_string = [&](const string &name, const string &def) {
		...
		return it != parameters.named_parameters.end() ? StringValue::Get(it->second) : def;
	};
	auto get_bool = [&](const string &name, bool def) {
		auto it = parameters.named_parameters.find(Identifier(name));
		return it != parameters.named_parameters.end() ? BooleanValue::Get(it->second) : def;
	};

	const string stemmer      = get_string("stemmer", "porter");
	const string stopwords    = get_string("stopwords", "english");
	const string ignore       = get_string("ignore", "[0-9!@#$%^&*()_+={}\\[\\]:;<>,.?~\\\\/\\|''\"`-]+");
	...
	if (stopwords != "english" && stopwords != "none") {
		...
		Catalog::GetEntry<TableCatalogEntry>(context, sw_qname.catalog, sw_qname.schema, sw_qname.name);
	}

	const string fts_schema = GetFTSSchema(qname);
	if (Catalog::GetSchema(context, qname.catalog, Identifier(GetFTSSchemaName(qname)),
	                       OnEntryNotFound::RETURN_NULL) && !overwrite) {
	...
	for (idx_t i = 2; i < parameters.values.size(); i++) {
		...
		if (!table.ColumnExists(Identifier(col_name))) {
			throw CatalogException("Table '%s.%s' does not have a column named '%s'!",
			                       qname.schema.GetIdentifierName(), qname.name.GetIdentifierName(), col_name);
		}
